...
contract StakingFacet{
    StakingAppStorage.StakingStorage internal s;

    IERC20 private stakeToken;
    IERC20 private rewardToken;

event Unstaked(address indexed _sender,address indexed _to, uint _amountToBeTransfered);
event StakingSuccessful(address sender, uint _amount);
    ...
    function init(address _stakedToken, address _rewardToken) external{
       ...
       function stake(uint _amount) external {
        ...
        require(IERC20(stakeToken).balanceOf(msg.sender) >= _amount, "insuffiecient balance");
    ...
    function calculateBalanceWithInterest(address _user) private view returns (uint256) {
        ...
        uint256 interest = (s.balanceOf[_user] * 120 * partialYearPercentage) / 10000; // Adjusted for partial year
    ...
    function unstaked() external{
           ...
           IERC20(rewardToken).transfer(msg.sender, _totalReward);
